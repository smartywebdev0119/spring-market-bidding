...
public interface AuctionRepo extends JpaRepository<Auction, Long> {
  ...
  @Query(value = "SELECT * FROM auctions WHERE title LIKE %:searchQuery% OR description LIKE %:searchQuery% ORDER BY timestamp DESC", nativeQuery = true)
  List<Auction> findAllAuctionBySearchQuery(@Param("searchQuery") String searchQuery);

  List<Auction> findByOrderByTimestampDesc();
}